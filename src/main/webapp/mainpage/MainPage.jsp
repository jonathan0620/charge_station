	...
	<div id="map-container">
		...
		<div id="panel">
			...
			<div id="toggle-btn-test">
				<button class="btn bg-light" id="bt-test"
					style="background-color: #f1f1f1">닫기</button>
			</div>
			<!-- 검색 결과 목록 -->
     ...
     // 사용자 마커찍기 
        ...
        function renderSearchResults(response) {
            ...
            for (let i = 0; i < response.length; i++) {
                resultDiv.append('<div class="result-item"><input type="hidden" class="location-info" data-lat="' + response[i].es_lat + '" data-lon="' + response[i].es_lon + '" data-name="' + response[i].es_statNm + '" data-statid="' + response[i].es_statId + '" data-stat="' + response[i].stat + '">'
        ...
        function renderSearchResults(results) {
            ...
            if (results.length === 0) {
                ...
                for (let i = 0; i < results.length; i++) {
                    // 결과를 추가하는 부분
                    resultlist.append('<div class="result-item"><input type="hidden" class="location-info" data-lat="' + results[i].es_lat + '" data-lon="' + results[i].es_lon + '" data-name="' + results[i].es_statNm + '" data-stat="' + results[i].stat + '" data-statid="' + results[i].es_statId + '">'
                        + '<a href="#" class="result-link text-decoration-none">'
                        + results[i].es_statNm
                        + '</a>' + '</div>');
                }
        ...
        function apiDetailRequest(es_statId) {
            ...
            $.ajax({
                ...
                success: function(json) {
                    ...
                    var statNm = statList[0].statNm;
                    var addr = statList[0].addr;
                    var bnm = statList[0].bnm;
                    var busiCall = statList[0].busiCall;
                    var location = statList[0].location;
                    if (location !== null && location !== "null") {
                        location;
                    } else {
                        location = "정보가 없습니다";
                    }
                    var useTime = statList[0].useTime;
                    var busiNm = statList[0].busiNm;
                    var parkingFree = statList[0].parkingFree;
                    var trafficYn = statList[0].trafficYn
                    var note = statList[0].note;
                    var output = statList[0].output + 'kW';
                    ...
                    modalBody.append('<table class="table table-bordered" style="border-collapse: collapse; width: 95%; margin: 0 auto; background-color: #f2f2f2; !important">' + '<tr><td style="border: 1px solid #ddd;">도로명주소</td><td style="border: 1px solid #ddd;">' + addr + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">기관명</td><td style="border: 1px solid #ddd;">' + bnm + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">전화번호</td><td style="border: 1px solid #ddd;">' + busiCall + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">상세위치</td><td style="border: 1px solid #ddd;">' + location + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">운영기관명</td><td style="border: 1px solid #ddd;">' + busiNm + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">주차료 무료</td><td style="border: 1px solid #ddd;">' + parkingFree + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">편의제공 여부</td><td style="border: 1px solid #ddd;">' + trafficYn + '</td></tr>' + '<tr><td style="border: 1px solid #ddd;">상세</td><td style="border: 1px solid #ddd;">' + note + '</td></tr>' + '</table>');
                    modalBody.append('<br>');
